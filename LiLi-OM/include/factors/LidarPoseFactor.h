...
public:
    ...
    template <typename T> bool operator()(const T *p1, const T *p2, const T *p3, const T *p4, T *residuals) const {
        ...
        residuals[0] = T(1) * residual1[0];
        residuals[1] = T(1) * residual1[1];
        residuals[2] = T(1) * residual1[2];
        residuals[3] = T(1) * residual2[0];
        residuals[4] = T(1) * residual2[1];
        residuals[5] = T(1) * residual2[2];

        return true;
    }

    static ceres::CostFunction *Create(Eigen::Quaterniond delta_q_, Eigen::Vector3d delta_p_) {
        return (new ceres::AutoDiffCostFunction<LidarPoseFactorAutoDiff, 6, 3, 4, 3, 4>(new LidarPoseFactorAutoDiff(delta_q_, delta_p_)));
    }
...
public:
    ...
    template <typename T> bool operator()(const T *p1, const T *p2, T *residuals) const {
    ...
    static ceres::CostFunction *Create(Eigen::Quaterniond delta_q_, Eigen::Vector3d delta_p_, Eigen::Quaterniond ql_, Eigen::Vector3d pl_) {
        return (new ceres::AutoDiffCostFunction<LidarPoseLeftFactorAutoDiff, 6, 3, 4>(new LidarPoseLeftFactorAutoDiff(delta_q_, delta_p_, ql_, pl_)));
    }
...
public:
    ...
    static ceres::CostFunction *Create(Eigen::Quaterniond delta_q_, Eigen::Vector3d delta_p_, Eigen::Quaterniond qr_, Eigen::Vector3d pr_) {
        return (new ceres::AutoDiffCostFunction<LidarPoseRightFactorAutoDiff, 6, 3, 4>(new LidarPoseRightFactorAutoDiff(delta_q_, delta_p_, qr_, pr_)));
    }
